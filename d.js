...
$(document).ready(function () {
    ...
    $("input[isnum='true']").on("keyup keydown", function (e) {
        entNumber(e, 1)
    }); //只能输入(不包含小数点)数字
    $("input[isnums='true']").on("keyup keydown", function (e) {
        entNumber(e)
    }); //只能输入(或包含小数点)数字
        ...
        ToolBoxAdd: function () {
            ...
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                ...
                $.ajax({
                    type: "POST",
                    url: "/ajaxsync.aspx",
                    data: 'at=toolbox&iswords=' + iswords + '&addval=' + escape(val)
                });
        },
        ...
        ToolBoxDeleteValue: function (val) {
            ...
            if (location.host.indexOf("mobile") >= 0)//如果是mobile.chinaz.com
                ...
                $.ajax({
                    type: "POST",
                    url: "/ajaxsync.aspx",
                    data: 'at=toolbox&iswords=' + iswords + '&delval=' + escape(val)
                });
        },
